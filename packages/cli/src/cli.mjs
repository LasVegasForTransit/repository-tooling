 ...
 *   lvbt deploy     build, then `wrangler deploy`
 *
 * A repository's package.json maps its standard scripts to these, so
 * `pnpm bootstrap`, `pnpm preflight`, and `pnpm deploy` behave identically
 * across the organization. Everything else a repository needs is an ordinary
 * dependency or an ordinary file it owns.
 */
...
const commands = { bootstrap, preflight, deploy };
